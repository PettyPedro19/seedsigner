        ...
        def run(self):
            ...
            while self.keep_running:
                ...
                if is_brightness_tip_enabled and time.time_ns() - self.tips_start_time.cur_count < duration:
                    image = self.qr_encoder.part_to_image(self.qr_encoder.cur_part(), self.renderer.canvas_width, self.renderer.canvas_width, border=2, background_color=hex_color) # FIXED: now using canvas_width instead of static value.
                    ...
                    if pending_encoder_restart:
                        ...
                        pending_encoder_restart = False
                    image = self.qr_encoder.next_part_image(self.renderer.canvas_width, self.renderer.canvas_width, border=2, background_color=hex_color) #FIXED: same as above. Replaced static dimesnions (240x240) with canvas_width.
