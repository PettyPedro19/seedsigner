...
#
#  macOS fix: tkinter must run on the main thread.
#  GUI setup moved to start_gui() which is called from main.py on the main thread.
#  ShowImage() uses root.after() for thread-safe updates from the controller thread.
...
class desktopDisplay:
    """Desktop display driver. GUI runs on the main thread; controller runs on a background thread."""

    _instance = None          # set in __init__ so main.py can find it
    _gui_ready = threading.Event()  # set once mainloop is running

    root = None

    def __init__(self, display_type: str = DISPLAY_TYPE__ST7789, width: int = 240, height: int = 320):
        self.width = width
        self.height = height
        self.display_type = display_type
        self.label = None
        desktopDisplay._instance = self   # register so main.py can pick it up

    # ------------------------------------------------------------------
    # Called from the MAIN THREAD (main.py) after the controller thread
    # has created this instance.
    # ------------------------------------------------------------------
    def start_gui(self):
        """Initialise tkinter and run mainloop – MUST be called from the main thread."""
        self.root = tk.Tk()
        ...
        # Signal that the GUI is ready so ShowImage() calls can proceed
        desktopDisplay._gui_ready.set()
    ...
    # ------------------------------------------------------------------
    # Thread-safe image update: schedule on the main thread via after()
    # ------------------------------------------------------------------
    def _do_show_image(self, Image2):
    ...
    def ShowImage(self, Image2, Xstart, Ystart):
        # Wait until the GUI is fully initialised
        desktopDisplay._gui_ready.wait()
        ...
        if imwidth != self.width or imheight != self.height:
            if self.display_type == DISPLAY_TYPE__ILI9341:
                ...
                raise ValueError(
                    'Image must be same dimensions as display ({0}x{1}).'.format(self.width, self.height)
                )

        # Schedule the actual tkinter update on the main thread
        self.root.after(0, self._do_show_image, Image2)

    show_image = ShowImage  # alias for SeedSigner PR #741 compatibility

    def update_geometry(self):
        """Update window size dynamically (PR #741 compatibility)."""
        if self.root:
            self.root.after(0, self._do_update_geometry)
    ...
    def invert(self, enabled: bool = True):
        """Invert display colours (PR #741 compatibility – no-op on emulator)."""
        pass
